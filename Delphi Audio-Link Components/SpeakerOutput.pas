...
type
  ...
  TToneThread = class(TThread)
  private
    FSpeakerOutput: TObject;
    FFrequency: Integer;
    FDuration: Integer;
  public
    constructor Create(ASpeakerOutput: TObject; AFrequency, ADuration: Integer);
  ...
  public
    ...
    // NEW: TBytes-based methods for network compatibility
    procedure PlayBufferBytes(const Buffer: TBytes);
    procedure PlayBuffer(Buffer: PByte; Size: Integer); // Keep original for compatibility
    procedure PlaySound(Frequency: Integer; Duration: Integer);
...
// NEW: TBYTES-BASED PLAYBUFFER WITH INTELLIGENT FORMAT CONVERSION
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        // SMART FORMAT HANDLING - Windows AUTOCONVERTPCM will handle most conversions
        // But we still apply volume control at the correct bit depth
...
// ORIGINAL PLAYBUFFER FOR BACKWARD COMPATIBILITY
...
procedure TSpeakerOutput.PlaySound(Frequency: Integer; Duration: Integer);
begin
  if not FActive then Exit;

  // Create thread to play tone - NO UI BLOCKING
  TToneThread.Create(Self, Frequency, Duration);
end;

// TToneThread - SEPARATE THREAD FOR TONE GENERATION
constructor TToneThread.Create(ASpeakerOutput: TObject; AFrequency, ADuration: Integer);
begin
  FSpeakerOutput := ASpeakerOutput;
...
procedure TToneThread.Execute;
var
  Speaker: TSpeakerOutput;
  SampleCount: Integer;
  Buffer: TBytes;
  I, J: Integer;
  Sample: SmallInt;
  ...
  BytesPerFrame: Integer;
begin
  Speaker := TSpeakerOutput(FSpeakerOutput);

  if not Speaker.Active or not Assigned(Speaker.FWaveFormat) then Exit;

  SampleRate := Speaker.FWaveFormat^.nSamplesPerSec;
  Channels := Speaker.FWaveFormat^.nChannels;
  BytesPerFrame := Speaker.FWaveFormat^.nBlockAlign;

  SampleCount := (SampleRate * FDuration * Channels) div 1000;
  SetLength(Buffer, SampleCount * 2); // 16-bit samples

  // Generate tone with volume applied
  for I := 0 to (SampleCount div Channels) - 1 do
  begin
    Angle := 2.0 * Pi * FFrequency * I / SampleRate;
    Sample := Round(Sin(Angle) * 16000 * Speaker.FVolumeFactor);

    for J := 0 to Channels - 1 do
    begin
      PSmallInt(@Buffer[I * Channels * 2 + J * 2])^ := Sample;
    end;
  end;

  // Play using new TBytes method
  ChunkSize := (SampleRate * Channels * 2) div 20; // 50ms chunks in bytes
  Position := 0;

  while (Position < Length(Buffer)) and not Terminated do
  begin
    var RemainingBytes := Length(Buffer) - Position;
    var ChunkBytes := Min(ChunkSize, RemainingBytes);

    var ChunkBuffer: TBytes;
    SetLength(ChunkBuffer, ChunkBytes);
    Move(Buffer[Position], ChunkBuffer[0], ChunkBytes);

    Speaker.PlayBufferBytes(ChunkBuffer);
    Position := Position + ChunkBytes;

    Sleep(25); // Small delay between chunks
  end;
end;
