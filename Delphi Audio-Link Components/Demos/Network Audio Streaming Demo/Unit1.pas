...
type
  TForm1 = class(TForm)
    ...
    // NEW: Audio Injection Controls
    GroupBox3: TGroupBox;
    chkMixInjectedAudio: TCheckBox;
    lblInjectedVolume: TLabel;
    trkInjectedVolume: TTrackBar;
    lblInjectedVolumeValue: TLabel;
    lblBeepFreq: TLabel;
    edtBeepFreq: TEdit;
    lblBeepDuration: TLabel;
    edtBeepDuration: TEdit;
    btnInjectBeep: TButton;
    btnInjectAudioFile: TButton;
    btnStopInjection: TButton;
    btnTestInjection: TButton;
    chkLoopAudio: TCheckBox;
    OpenDialog1: TOpenDialog;
    chkVirtualMicMode: TCheckBox;
    ...
    // NEW: Audio Injection Event Handlers
    procedure chkVirtualMicModeClick(Sender: TObject);
    procedure chkMixInjectedAudioClick(Sender: TObject);
    procedure trkInjectedVolumeChange(Sender: TObject);
    procedure btnInjectBeepClick(Sender: TObject);
    procedure btnInjectAudioFileClick(Sender: TObject);
    procedure btnStopInjectionClick(Sender: TObject);
    procedure btnTestInjectionClick(Sender: TObject);
  ...
  private
    ...
    procedure SetupInjectionControls; // NEW
  public
    { Public declarations }
  end;
...
begin
  ...
  // Initialize UI
  UpdateUI;
  LogMessage('Network Audio Streaming Demo with Injection initialized');
  LogMessage('Protocol: TCP=Reliable transmission, UDP=Fast/minimal latency');
  LogMessage('NEW: Audio injection features - virtual mic, file/beep injection');
...
// NEW: Setup injection controls
procedure TForm1.SetupInjectionControls;
begin
  // Setup injection volume control
  trkInjectedVolume.Min := 0;
  trkInjectedVolume.Max := 100;
  trkInjectedVolume.Position := 100;
  lblInjectedVolumeValue.Caption := '100%';

  // Setup beep defaults
  edtBeepFreq.Text := '800';
  edtBeepDuration.Text := '1000';

  // Setup checkboxes
  chkMixInjectedAudio.Checked := True;
  chkLoopAudio.Checked := False;

  // Setup file dialog
  OpenDialog1.Filter := 'WAV Files (*.wav)|*.wav|All Files (*.*)|*.*';
  OpenDialog1.Title := 'Select Audio File for Injection';
end;
...
begin
  ...
  if FIsMicActive then
  begin
    if chkVirtualMicMode.Checked then
      lblMicStatus.Caption := 'Virtual Microphone: Active'
    else
...
begin
  ...
  cmbMicDevice.Enabled := not FIsMicActive and not chkVirtualMicMode.Checked;
  ...
  // NEW: Injection control states
  chkVirtualMicMode.Enabled := not FIsMicActive;
  chkMixInjectedAudio.Enabled := not chkVirtualMicMode.Checked;
  btnInjectBeep.Enabled := FIsMicActive;
  btnInjectAudioFile.Enabled := FIsMicActive;
  btnStopInjection.Enabled := FIsMicActive;
  btnTestInjection.Enabled := FIsMicActive;
  edtBeepFreq.Enabled := FIsMicActive;
  edtBeepDuration.Enabled := FIsMicActive;
  chkLoopAudio.Enabled := True;
end;
...
// NEW: Audio Injection Event Handlers
procedure TForm1.chkVirtualMicModeClick(Sender: TObject);
begin
  try
    MicInput1.VirtualMicMode := chkVirtualMicMode.Checked;
    if chkVirtualMicMode.Checked then
    begin
      LogMessage('Virtual microphone mode enabled - hardware mic bypassed');
      chkMixInjectedAudio.Checked := False;
    end
    else
    begin
      LogMessage('Normal microphone mode - uses hardware mic');
      chkMixInjectedAudio.Checked := True;
    end;
    MicInput1.MixInjectedAudio := chkMixInjectedAudio.Checked;
    UpdateUI;
  except
    on E: Exception do
      LogMessage('Error setting virtual mic mode: ' + E.Message);
  end;
end;

procedure TForm1.chkMixInjectedAudioClick(Sender: TObject);
begin
  try
    MicInput1.MixInjectedAudio := chkMixInjectedAudio.Checked;
    if chkMixInjectedAudio.Checked then
      LogMessage('Injected audio will MIX with microphone')
    else
      LogMessage('Injected audio will REPLACE microphone');
  except
    on E: Exception do
      LogMessage('Error setting mix mode: ' + E.Message);
  end;
end;

procedure TForm1.trkInjectedVolumeChange(Sender: TObject);
begin
  try
    MicInput1.InjectedVolume := trkInjectedVolume.Position;
    lblInjectedVolumeValue.Caption := IntToStr(trkInjectedVolume.Position) + '%';
  except
    on E: Exception do
      LogMessage('Error setting injection volume: ' + E.Message);
  end;
end;

procedure TForm1.btnInjectBeepClick(Sender: TObject);
begin
  try
    var Freq := StrToIntDef(edtBeepFreq.Text, 800);
    var Duration := StrToIntDef(edtBeepDuration.Text, 1000);

    LogMessage(Format('Injecting beep: %dHz for %dms', [Freq, Duration]));
    MicInput1.InjectBeep(Freq, Duration);
    LogMessage('Beep injection started - will stream over network');
  except
    on E: Exception do
      LogMessage('Error injecting beep: ' + E.Message);
  end;
end;

procedure TForm1.btnInjectAudioFileClick(Sender: TObject);
begin
  try
    if OpenDialog1.Execute then
    begin
      LogMessage('Injecting audio file: ' + ExtractFileName(OpenDialog1.FileName));
      MicInput1.InjectAudioFile(OpenDialog1.FileName, chkLoopAudio.Checked);
      LogMessage('Audio file injection started - streaming over network (Loop: ' + BoolToStr(chkLoopAudio.Checked, True) + ')');
    end;
  except
  on E: Exception do
     LogMessage('Error injecting audio file: ' + E.Message);
 end;
end;

procedure TForm1.btnStopInjectionClick(Sender: TObject);
begin
 try
   LogMessage('Stopping audio injection...');
   MicInput1.StopInjection;
   LogMessage('Audio injection stopped');
 except
   on E: Exception do
     LogMessage('Error stopping injection: ' + E.Message);
 end;
end;

procedure TForm1.btnTestInjectionClick(Sender: TObject);
begin
 try
   LogMessage('=== TESTING NETWORK AUDIO INJECTION ===');

   if FIsMicActive then
   begin
     LogMessage('Testing injection beep over network...');
     MicInput1.InjectBeep(1200, 800);
     LogMessage('Injected 1200Hz beep for 800ms - streaming to server');

     Sleep(1000);

     LogMessage('Testing different frequency...');
     MicInput1.InjectBeep(600, 500);
     LogMessage('Injected 600Hz beep for 500ms - streaming to server');
   end
   else
     LogMessage('Microphone not active - start mic first to test injection');

   LogMessage('=== NETWORK INJECTION TEST COMPLETE ===');
 except
   on E: Exception do
     LogMessage('Error in injection test: ' + E.Message);
 end;
end;
...
begin
 try
   // Apply injection settings before starting
   MicInput1.VirtualMicMode := chkVirtualMicMode.Checked;
   MicInput1.MixInjectedAudio := chkMixInjectedAudio.Checked;
   MicInput1.InjectedVolume := trkInjectedVolume.Position;
   ...
   if chkVirtualMicMode.Checked then
     LogMessage('Virtual microphone started - injection-only mode')
   else
     LogMessage('Microphone started - hardware + injection streaming');
...
begin
 try
   ...
   MicInput1.Active := False;
   FIsMicActive := False;
   LogMessage('Microphone stopped - audio injection stopped');
   UpdateUI;
 except
   on E: Exception do
...
begin
 try
   SpeakerOutput1.Active := True;
   FIsSpeakerActive := True;
   LogMessage('Speaker started - ready to receive network audio');
